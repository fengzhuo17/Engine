...
void ResetStreamObj( PStreamObject pObj ){
    ...
    if ( (pObj->type == OBJTYPE)&&(pObj->size == sizeof(StreamObject)) ){
        ...
        pObj->scanTree = NULL;
        //pObj->DBlib = global_root;

        pObj->DBlib2 = (PTree)gPeerArrayRoot;
...
PE_RECEIVED:
    ...
    if ( pInfo->status == PE_STATUS_PE_RECEIVED ){
        //now collecting PE infomation begins
...
void SuspectScan( PStreamObject pObj, const BYTE* pBuff ){

    PTree lib = pObj->DBlib2;
...
LONG GeneralDataScan( PStreamObject pObj, const BYTE* pbDataBuff, DWORD Length, DWORD *aRet ){
    ...
    if (OBJSTATUS_SCANVIRUS_PENDING == pObj->status){
        ...
        SuspectScan( pObj, pbDataBuff );

        if ( pObj->DBfound ){/* NOT COMPLETED */

            pObj->status = OBJSTATUS_SCANVIRUS_PENDING;

            if ( gReport ){
                
                gReport( pObj->DBfound, gReportArg );
            }

            *aRet = pObj->DBindex ;

            return pObj->DBfound;//911
            
        }else if ( pObj->DBlib2 ){

            goto end;

        }else if( pObj->DBlib2==NULL ){

            return 0;// no DB found
        }
    }
...
realfind:
    ...
    if ( OBJSTATUS_SCANVIRUS_PENDING == pObj->status ){
        ...
        if ( scanresult == 0 ){
            ...
            printf( "Attachment : No Suspect\n");
        }
        
        if ( scanresult > 0 ){

            printf( "Attachment has DB id: %d\n", scanresult );
