...
void InternalScan3( PStreamObject pObj, const BYTE* buff ){
	...
	PeerArray* curr = (PeerArray*)pObj->suspectlib2;
	Element* lFound = NULL;
	...
	do{
		...
		if ( lFound ){
			...
			if ( curr == NULL ){
				...
				//pObj->suspectfound = lFound->id;
				PInfoNode tmp = NULL;
				
				pObj->suspectindex = (DWORD)lFound - (DWORD)gPeerAddress.base;


				tmp = FindInfoNode( pObj->suspectindex );

				if (tmp){
					
					if ( tmp->Type == 0 )pObj->suspectfound = tmp->u.v.VID;
...
void InternalScanMisc( PStreamObject pObj, const BYTE* buff ){
	...
	PIndexElementArray curr = (PIndexElementArray)pObj->suspectlib2;
