...
#include "suspectlib_anly.h"

#include "suspectimg.h"
...
Tree DBTreeRoot = { NULL, NULL, NULL};
...
BOOL DBRecProcess( PLibRec pRec){
...
DWORD  LoadRawDB(const char* pLibPath, DWORD Protocol, DWORD max ){
...
void BuildTotalPeerArray(){
	...
	lVirtualNode.node.left = DBTreeRoot.left;
...
BOOL ZBuildImgForPeerArray( PMemAlloc aPtr, CHAR* aDir ){
	...
	WriteFile( hFile, &ImgHead, sizeof(DBImgHead),&num, NULL );	
